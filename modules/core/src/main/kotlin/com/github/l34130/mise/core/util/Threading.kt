...
fun canSafelyInvokeAndWait(): Boolean {
    ...
    if (threadName.contains("DefaultDispatcher-worker")) {
        logger.debug("Detected coroutine dispatcher thread, cannot safelyInvokeAndWait")
        return false
    }
    ...
    if (hasInfrastructure) {
        logger.debug("Detected IDE infrastructure caller in stack, cannot safelyInvokeAndWait")
        return false
    }
