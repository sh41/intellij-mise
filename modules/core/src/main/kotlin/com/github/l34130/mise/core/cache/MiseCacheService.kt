...
class MiseCacheService(private val project: Project) {
    ...
    /**
     * Cache for executable paths (resolved, auto-detected)
     * Small capacity - typically only 2-3 entries per project.
     */
    private val executableCache: Cache<String, String> = Caffeine.newBuilder()
        .maximumSize(10)
        .removalListener { key: String?, _: String?, cause ->
            if (cause == RemovalCause.SIZE) {
     ...
     */
    fun <T : Any> getCachedCommand(key: String, compute: () -> T): T {
     ...
     */
    fun getOrComputeExecutable(key: String, compute: () -> String): String {
    ...
    fun getCachedExecutable(key: String): String? {
